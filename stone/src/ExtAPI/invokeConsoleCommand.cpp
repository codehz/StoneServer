...
static void invokeConsoleCommandCallback(FunctionCallbackInfo<Value> const &info) {
  ...
  if (info.Length() != 2) {
    iso->ThrowException(Exception::TypeError(ToJS(strformat("invokeConsoleCommand requires 2 arguments(current: %d)", info.Length()))));
    return;
  }
  if (!info[0]->IsString() || !info[1]->IsString()) {
    iso->ThrowException(Exception::TypeError(ToJS("invokeConsoleCommand requires (string, string)")));
    return;
  }
