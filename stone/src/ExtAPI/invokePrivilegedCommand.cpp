...
static void invokePrivilegedCommandCallback(FunctionCallbackInfo<Value> const &info) {
  ...
  if (info.Length() != 2) {
    iso->ThrowException(Exception::TypeError(ToJS(strformat("invokePrivilegedCommand requires 2 arguments(current: %d)", info.Length()))));
    return;
  }

  if (!info[0]->IsObject() || !info[1]->IsString()) {
    iso->ThrowException(Exception::TypeError(ToJS("invokePrivilegedCommand requires (object, string)")));
    return;
  }
  ...
  if (!actor || *(void **)actor != BUILD_HELPER(GetAddress, void, 0x8, "_ZTV12ServerPlayer").Address()) {
    iso->ThrowException(Exception::TypeError(ToJS("invokePrivilegedCommand requires (player, string)")));
    return;
  }
