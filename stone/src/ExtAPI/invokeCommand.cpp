...
static void invokeCommandCallback(FunctionCallbackInfo<Value> const &info) {
  ...
  if (info.Length() == 1) {
    if (!info[0]->IsString()) {
      iso->ThrowException(Exception::TypeError(STR("invokeCommand requires (string)")));
      return;
    }
    if (!current_orig) {
      iso->ThrowException(Exception::ReferenceError(STR("invokeCommand need be invoked inside custom command execution context")));
      return;
    }
    ...
    if (!info[0]->IsObject() || !info[1]->IsString()) {
      iso->ThrowException(Exception::ReferenceError(STR("invokeCommand requires (object, string)")));
      return;
    }
    ...
    if (!actor || *(void **)actor != BUILD_HELPER(GetAddress, void, 0x8, "_ZTV12ServerPlayer").Address()) {
      iso->ThrowException(Exception::ReferenceError(STR("invokeCommand requires (player, string)")));
      return;
    }
    ...
    iso->ThrowException(Exception::ReferenceError(STR(strformat("invokeCommand requires 1 or 2 arguments(current: %d)", info.Length()))));
    return;
  }
}
