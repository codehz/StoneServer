...
static void transferPlayerCallback(FunctionCallbackInfo<Value> const &info) {
  ...
  if (info.Length() != 3) {
    iso->ThrowException(Exception::TypeError(STR(strformat("transferPlayer requires 3 arguments(current: %d)", info.Length()))));
    return;
  }
  if (!info[0]->IsObject() || !info[1]->IsString() || !info[2]->IsInt32()) {
    iso->ThrowException(Exception::TypeError(STR("transferPlayer requires (object, string, integer)")));
    return;
  }
  Actor *actor = fromJS<Actor *>(iso, info[0]);
  if (!actor || *(void **)actor != BUILD_HELPER(GetAddress, void, 0x8, "_ZTV12ServerPlayer").Address()) {
    iso->ThrowException(Exception::TypeError(STR("transferPlayer requires (player, string, integer)")));
    return;
  }
