...
static void invokeCommandCallback(FunctionCallbackInfo<Value> const &info) {
  ...
  if (info.Length() == 1) {
    ...
    if (!info[0]->IsObject() || !info[1]->IsString()) {
      iso->ThrowException(Exception::TypeError(STR("invokeCommand requires (object, string)")));
      return;
    }
    ...
    if (!actor || *(void **)actor != BUILD_HELPER(GetAddress, void, 0x8, "_ZTV12ServerPlayer").Address()) {
      iso->ThrowException(Exception::TypeError(STR("invokeCommand requires (player, string)")));
      return;
    }
    ...
    iso->ThrowException(Exception::TypeError(STR(strformat("invokeCommand requires 1 or 2 arguments(current: %d)", info.Length()))));
    return;
  }
}
